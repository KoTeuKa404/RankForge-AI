...
export default function InternalLinkWorkspace({ authenticated, selectedProject, projects, onSelectProject }: Props) {
  ...
  return <div className="content-panel link-workspace">
    <div className="panel-heading"><div><div className="eyebrow">Internal linking agent</div><h2>Find contextual paths to underlinked pages</h2></div><div className="live-pill"><span/> Crawl-grounded</div></div>
    <div className="link-layout">
      <section className="link-control-card">
        ...
        <p>Only missing links between indexable pages are considered. Suggestions below the relevance threshold are excluded.</p>
      </section>
    ...
    {analysis && <>
      <section className="link-summary"><div><div className="eyebrow">Link plan ready</div><h3>{analysis.rootUrl}</h3><p>{analysis.pageCount} indexable pages evaluated</p></div><button className="button subtle" onClick={() => download("rankforge-internal-links.csv", internalLinksCsv(analysis), "text/csv;charset=utf-8")}>Export CSV</button></section>
